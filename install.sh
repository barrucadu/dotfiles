...
install -D -m677 rc.conf ${DESTDIR}/etc/rc.conf || exit 1
install -D -m644 functions ${DESTDIR}/etc/rc.d/functions || exit 1

# We're probably running this from GNU/Linux, to install over the top of the current initscripts
if [[ ! -e ${DESTDIR}/sbin/minilogd ]]; then
    gcc $CFLAGS -o minilogd minilogd.c || exit 1
    install -D -m755 minilogd ${DESTDIR}/sbin/minilogd || exit 1
fi

ln -s ${DESTDIR}/libexec/rc.shutdown ${DESTDIR}/sbin/halt
ln -s ${DESTDIR}/libexec/rc.shutdown ${DESTDIR}/sbin/reboot
